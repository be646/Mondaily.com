...
router.get("/token", async (c) => {
  ...
  if (!secret || !url || !anonKey) return c.json({ enabled: false, _missing: { SUPABASE_JWT_SECRET: !secret, SUPABASE_URL: !url, SUPABASE_ANON_KEY: !anonKey }, _secretLen: (secret ?? "").length });
