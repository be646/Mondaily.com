...
create table if not exists project_log (
  ...
  title       text not null,
  detail      text,
  -- update: free-text area (e.g. "AI gateway"); roadmap: tier
  -- (must_fix | should_improve | future)
  category    text,
...
-- Server reads/writes via the service-role key (RLS is bypassed there); the
-- frontend only ever goes through the API, never Supabase directly.
alter table project_log enable row level security;

-- Seed only if empty, so re-running the migration is safe.
insert into project_log (kind, title, detail, category, status, sort_order, created_at, completed_at)
select * from (values
  -- ───────────────── Recent updates (newest first via sort_order) ─────────────────
  ...
  ('update', 'Background agents run via Vercel Cron + on-demand',
   'Extracted every job into provider-agnostic runners driven by Inngest, a daily Vercel Cron (/api/cron/daily), and on-demand routes — no extra account needed.',
  ...
  ('update', 'Home rebuilt as an AI control room',
   ...
   'Home, Agent Constellation, Command Center', 'shipped', 6, '2026-06-21', '2026-06-21'),
  ('update', 'Prospecting Agent shipped',
   'Web-search-backed candidate discovery with dedupe and Decision Queue approval.',
   'routes/prospecting.ts, Ask tool, Lists', 'shipped', 7, '2026-06-21', '2026-06-21'),
  ('update', 'Decision Queue + Agent Registry',
   'Real approve/reject/snooze flow backing every agent''s "needs approval" state.',
   'decision_queue, /api/v1/agents, /api/v1/decisions', 'shipped', 8, '2026-06-19', '2026-06-19'),

  -- ───────────────── What''s next: must fix ─────────────────
  ...
  ('roadmap', 'Set CRON_SECRET + rotate exposed Cerebras key',
  ...
  -- ───────────────── What''s next: future AI-native ─────────────────
  ('roadmap', 'Workflow Agent execution engine',
   'Real trigger -> condition -> action engine so saved workflows actually run autonomously (currently design/CRUD only).',
...
where not exists (select 1 from project_log);
