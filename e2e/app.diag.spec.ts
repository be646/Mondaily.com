...
test("react2", async ({ page }) => {
  ...
  const out = await page.evaluate(async () => {
    ...
    const due = new Date(Date.now() - 16*86400000).toISOString();
    const t = await (await fetch(B+"/tasks",{method:"POST",credentials:"include",headers:H,body:JSON.stringify({title:"Recover stalled SkinCare.pl account before renewal deadline",priority:"urgent",status:"todo",due_date:due})})).json();
    ...
    return { task_id: t.id, title: d?.title, action: (d?.recommended_action||"").slice(0,200), confidence: rat?.confidence, rationale: (rat?.match_reason||"").slice(0,180) };
  });
  console.log("REACT2 " + JSON.stringify(out));
});
