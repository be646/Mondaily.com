...
export function MfaCard({ onSubmit, onBack, backLabel, trustOffered = true }: {
  ...
  return (
    <AuthShell kicker="Second factor" title="Two-factor code"
      ...
      footer={onBack ? <button onClick={onBack} className="transition-colors hover:text-[var(--text-secondary)]">{backLabel ?? "Back to password"}</button> : undefined}>
      <form onSubmit={submit} className="space-y-4">
        <input value={code} onChange={e => setCode(e.target.value)} autoFocus inputMode="text" autoComplete="one-time-code"
          ...
          className="w-full rounded-sm border border-[var(--border-soft)] bg-zinc-950 px-3.5 py-3 text-center font-mono text-[17px] tracking-[0.25em] text-[var(--text-primary)] outline-none transition-colors placeholder:text-[var(--text-muted)] focus:border-[#8fcf7f]" />
        {trustOffered && (
          <label className="flex cursor-pointer items-center gap-2.5 text-[11.5px] text-[var(--text-secondary)]">
