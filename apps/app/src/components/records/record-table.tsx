...
function StagePill({ value }: { value: string }) {
  ...
  const cls = map[value] ?? "bg-slate-500/15 text-slate-400";
  return <span className={`inline-flex rounded-md px-2 py-0.5 text-[11px] font-medium ${cls}`}>{value}</span>;
}
...
export function RecordTable({ objectType }: { objectType: string }) {
  ...
  const columns = Array.from(new Set(records.flatMap(r => Object.keys(r.data)))).slice(0, 8);
  ...
  function SortIcon({ col }: { col: string }) {
    ...
    return sortDir === "asc" ? <ChevronUp size={10} className="text-red-400 ml-1 shrink-0"/> : <ChevronDown size={10} className="text-red-400 ml-1 shrink-0"/>;
  }
  ...
  function renderCell(col: string, record: NodeRecord, colIndex: number) {
    ...
    if (col.toLowerCase().includes("stage") && typeof val === "string") {
      return <StagePill value={val}/>;
    }
    if (isFirst) {
      return (
          ...
          <tr>
            {columns.map(col => (
              <th key={col} className="whitespace-nowrap px-4 py-2 border-b border-white/[.05]">
                <button onClick={() => handleSort(col)} className={`flex items-center gap-1.5 hover:text-slate-300 transition-colors ${isNumeric(col) ? "ml-auto" : ""}`}>
                  {getColumnIcon(col)}
