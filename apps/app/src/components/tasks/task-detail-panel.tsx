...
import { X, Plus, Check, Trash2, Paperclip, Send } from "lucide-react";
import { useMutation, useQuery } from "@tanstack/react-query";
import { useUser } from "@clerk/react";
import { apiClient } from "../../lib/api-client";

interface Member { id: string; user_id: string; email: string; name: string; }
interface Task { id: string; title: string; labels?: string[]; notes?: string; }
...
const PERMISSIONS = ["edit", "comment", "view"];
...
export function TaskDetailPanel({ task, members, onClose, onUpdate }: {
  ...
  const addAssignee = useMutation({
    mutationFn: (m: Member & { permission: string }) => apiClient.post(`/tasks/${task.id}/assignees`, { user_id: m.user_id, email: m.email, name: m.name, permission: m.permission }),
  ...
  const tabs = [
    { key: "labels", label: "Labels", count: currentLabels.length },
            ...
            <div className="space-y-2">
              ...
              {LABELS.map(label => (
                <button key={label} onClick={() => toggleLabel(label)}
                  className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${currentLabels.includes(label) ? "bg-red-500/10 border border-red-500/20 text-red-400" : "border border-white/[.06] text-slate-400 hover:border-white/10 hover:text-white"}`}>
                  <div className={`h-4 w-4 rounded border flex items-center justify-center shrink-0 ${currentLabels.includes(label) ? "border-red-400 bg-red-400" : "border-white/20"}`}>
                    {currentLabels.includes(label) && <Check size={10} className="text-white"/>}
                  </div>
                  {label}
                  {label === "Need Review" && <span className="ml-auto text-[10px] text-slate-600">→ Review queue</span>}
                </button>
              ))}
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              {assignees.map(a => (
                <div key={a.user_id} className="flex items-center gap-3 rounded-lg border border-white/[.06] px-3 py-2.5">
                  ...
                  <button onClick={() => removeAssignee.mutate(a.user_id)} className="text-slate-600 hover:text-red-400"><X size={13}/></button>
                </div>
              ))}
              <div className="border-t border-white/[.06] pt-3 mt-3">
                <p className="text-xs text-slate-600 mb-2">Add member</p>
                {members.filter(m => !assignees.find(a => a.user_id === m.user_id)).map(m => (
                  <div key={m.user_id} className="flex items-center gap-2 py-1.5">
                    <div className="flex h-6 w-6 items-center justify-center rounded-full bg-white/10 text-xs text-slate-400 shrink-0">
                      {(m.name || m.email).charAt(0).toUpperCase()}
                    </div>
                    <span className="flex-1 text-sm text-slate-400 truncate">{m.name || m.email}</span>
                    {PERMISSIONS.map(p => (
                      <button key={p} onClick={() => addAssignee.mutate({ ...m, permission: p })}
                        className="rounded px-2 py-0.5 text-[11px] border border-white/10 text-slate-500 hover:border-red-500/30 hover:text-red-400 capitalize transition-colors">
                        {p}
                      </button>
                    ))}
                  </div>
                ))}
                {members.filter(m => !assignees.find(a => a.user_id === m.user_id)).length === 0 && (
              ...
              <div className="space-y-2 mb-3">
                {checklist.map(item => (
                  <div key={item.id} className="flex items-start gap-2.5 group">
                    ...
                    <button onClick={() => deleteCheckItem.mutate(item.id)} className="opacity-0 group-hover:opacity-100 text-slate-600 hover:text-red-400 transition-opacity">
                      <Trash2 size={12}/>
                    </button>
                  </div>
                ))}
          ...
          {activeTab === "attachments" && (
            <div>
                ...
                {attachments.map(a => (
                  <div key={a.id} className="flex items-center gap-3 rounded-lg border border-white/[.06] px-3 py-2.5 group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-slate-600">{a.user_name} · {(a.file_size / 1024).toFixed(1)}KB</p>
                    </div>
              ...
              <div className="rounded-lg border border-dashed border-white/10 p-4 text-center">
                <Paperclip size={16} className="text-slate-600 mx-auto mb-2"/>
                <p className="text-xs text-slate-500 mb-2">Paste a file URL to attach</p>
                <div className="flex gap-2">
                  <input id="attach-url" placeholder="https://..." className="flex-1 h-8 rounded-lg border border-white/10 bg-transparent px-3 text-xs text-white placeholder-slate-600 outline-none"/>
                  <button onClick={() => {
                    const url = (document.getElementById("attach-url") as HTMLInputElement)?.value;
                    if (!url) return;
                    const name = url.split("/").pop() || "file";
                    apiClient.post(`/tasks/${task.id}/attachments`, { file_name: name, file_url: url, file_type: "link", file_size: 0, user_name: userName })
                      .then(() => { (document.getElementById("attach-url") as HTMLInputElement).value = ""; attachmentsQ.refetch(); });
                  }} className="h-8 px-3 rounded-lg bg-red-600 text-xs text-white">Add</button>
                </div>
              </div>
            </div>
          )}
