  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        <select value={role} onChange={e => setRole(e.target.value)} className="rounded-xl border border-black/[.08] bg-white px-3 py-1.5 font-mono text-[12px] text-stone-700 outline-none focus:border-indigo-500/40 transition-colors">
        ...
        {emails.map((email, i) => (
          <div key={i} className="flex gap-2">
            <input
              ...
              className="h-10 flex-1 rounded-xl border border-black/[.08] bg-white px-4 font-mono text-[13px] text-stone-900 placeholder-stone-400 outline-none focus:border-indigo-500/40 transition-colors"
            />
            {emails.length > 1 && (
