    ...
    <div className="rounded-2xl border border-black/[.08] bg-white p-8">
      ...
      <div className="mb-7 space-y-3">
        {(["gmail", "outlook"] as const).map(provider => (
          ...
          >
            <Mail size={16} className={connected.includes(provider) ? "text-indigo-500" : "text-zinc-400"} />
            <div className="flex-1">
              <p className="font-mono text-[12px] font-medium text-zinc-800">
                Connect {provider === "gmail" ? "Gmail" : "Outlook"}
              </p>
              <p className="font-mono text-[11px] text-zinc-500">Email and calendar</p>
            </div>
            {connected.includes(provider) && <CheckCircle size={15} className="text-indigo-500" />}
          </button>
        ))}
      </div>
