  ...
  if (!token) {
    return (
      <AuthShell kicker="Sovereign auth" title="Link required"
        ...
        footer={<button onClick={() => navigate("/auth/forgot")} className="inline-flex items-center gap-1.5 transition-colors hover:text-[var(--text-secondary)]"><ArrowLeft size={12} /> Request a reset link</button>}>
        <div className="flex items-center gap-2.5 rounded-sm border border-[var(--border-soft)] bg-zinc-950 px-3.5 py-3 text-[12.5px] text-[var(--text-secondary)]">
          <AlertTriangle size={16} className="text-[#c6892e]" /> Missing reset token.
        </div>
      </AuthShell>
    );
  }
