  ...
  return (
    <div className="relative min-h-full">
      <header className="flex flex-wrap items-center gap-3 border-b border-white/10 px-4 py-4 sm:px-6">
        ...
        <button onClick={() => setAnalysisOpen(true)} className="flex h-9 items-center gap-2 rounded-md bg-stone-600 px-3 text-sm font-medium"><Sparkles size={14} /> Run analysis</button>
      </header>
      <div className="grid min-h-[calc(100vh-74px)] lg:grid-cols-[minmax(320px,0.8fr)_minmax(480px,1.2fr)]">
        <section className="border-b border-white/10 p-4 sm:p-6 lg:border-b-0 lg:border-r">
          ...
          {call.linked_records.length ? <div className="mt-3 flex flex-wrap gap-2">{call.linked_records.map((record) => <Link key={record.id} to={`/objects/${record.object_type}/${record.id}`} className="rounded-full border border-white/10 px-2.5 py-1 text-xs text-stone-400">{record.name}</Link>)}</div> : null}
          ...
          <div className="rounded-lg border border-white/10 p-4">
            {call.audio_url ? <>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <button disabled={!ready} onClick={() => waveRef.current?.playPause()} className="grid h-9 w-9 place-items-center rounded-full bg-stone-600 text-white disabled:opacity-40">{playing ? <Pause size={15} /> : <Play size={15} />}</button>
                <Clock3 size={14} className="text-stone-600" />
                <select onChange={(event) => waveRef.current?.setPlaybackRate(Number(event.target.value))} className="h-8 rounded-md border border-white/10 bg-[#0d0d0d] px-2 text-xs"><option value="0.75">0.75x</option><option value="1" selected>1x</option><option value="1.25">1.25x</option><option value="1.5">1.5x</option><option value="2">2x</option></select>
