    ...
    <div className="relative min-h-full">
      ...
      <div className="grid min-h-[calc(100vh-74px)] lg:grid-cols-[minmax(320px,0.8fr)_minmax(480px,1.2fr)]">
        <section className="border-b border-white/10 p-4 sm:p-6 lg:border-b-0 lg:border-r">
          ...
          <div className="space-y-2">{call.participants.map((person) => person.id ? <Link key={person.id} to={`/objects/${person.object_type || "people"}/${person.id}`} className="flex items-center gap-3 rounded-md border border-white/10 p-3 hover:bg-white/[.025]"><div className="grid h-8 w-8 place-items-center rounded-full bg-white/10 text-xs">{person.name.slice(0, 2).toUpperCase()}</div><div><p className="text-sm">{person.name}</p><p className="text-xs text-stone-600">{person.email}</p></div></Link> : <div key={person.email || person.name} className="flex items-center gap-3 rounded-md border border-white/10 p-3"><div className="grid h-8 w-8 place-items-center rounded-full bg-white/10 text-xs">{person.name.slice(0, 2).toUpperCase()}</div><div><p className="text-sm">{person.name}</p><p className="text-xs text-stone-600">{person.email}</p></div></div>)}</div>
