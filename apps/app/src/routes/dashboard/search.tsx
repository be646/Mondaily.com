...
import { Building2, CheckSquare, Database, Search, UserRound, ArrowRight, Sparkles } from "lucide-react";
import { useEffect, useMemo, useRef, useState } from "react";
import { useNavigate, useSearchParams, Link } from "react-router-dom";
import { EmptyState, ConsoleSkeleton } from "../../components/ui/page-state";
import { apiClient } from "../../lib/api-client";

interface SearchResult { id: string; object_type: string; data: Record<string, unknown>; updated_at?: string; last_activity_at?: string; reason?: string | null }
...
export function SearchPage() {
  ...
  const results = useQuery({
    ...
    // Semantic mode: LLM-reranked retrieval (finds records by meaning, with a reason per hit).
    // Keyword mode: the fast literal match. Same result shape so the render is shared.
    queryFn: async () => {
      if (semantic) { const r = await apiClient.post<{ results: SearchResult[] }>("/search/semantic", { query, limit: 20 }); return r.results ?? []; }
      return apiClient.post<SearchResult[]>("/search", { query, limit: 50 });
    },
  ...
  return <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8" onKeyDown={(event) => { if (event.key === "ArrowDown") { event.preventDefault(); setSelected((value) => Math.min(value + 1, flat.length - 1)); } if (event.key === "ArrowUp") { event.preventDefault(); setSelected((value) => Math.max(value - 1, 0)); } if (event.key === "Enter" && flat[selected]) open(flat[selected]); }}>
    ...
    <div className="mt-3 flex items-center gap-2">
      <button onClick={() => setSemantic(false)} className="mode-pill" data-on={!semantic}><Search size={12} /> Keyword</button>
      <button onClick={() => setSemantic(true)} className="mode-pill" data-on={semantic}><Sparkles size={12} /> Semantic</button>
      <span className="text-[11px]" style={{ color: "var(--text-faint)" }}>{semantic ? "AI ranks by meaning, not just keywords" : "fast literal match"}</span>
    </div>
    <div className="mt-6">{!query ? <BrowseShortcuts types={schema.data ?? []} /> : results.isLoading ? <ConsoleSkeleton rows={7} /> : flat.length === 0 ? <EmptyState icon={Search} title={`No results for "${query}"`} description={semantic ? "The AI found nothing that matches the meaning of this query in your workspace." : "Try a company name, email address, deal, note, or a broader keyword."} action={<div className="flex gap-2 text-xs text-[var(--text-muted)]"><button onClick={() => setInput("")}>Clear search</button>{!semantic && <button onClick={() => setSemantic(true)}>Try semantic search</button>}<button onClick={() => navigate("/ask")}>Ask Mondaily</button></div>} /> : semantic ? <div className="minimal-sheet divide-y divide-[var(--border-soft)] overflow-hidden">{flat.map((item) => { const index = flat.indexOf(item); const name = String(item.data.name ?? item.data.title ?? item.data.subject ?? "Untitled"); const keyField = String(item.data.email ?? item.data.domain ?? item.data.company ?? item.data.value ?? ""); return <button key={item.id} onClick={() => open(item)} className={`flex w-full items-start gap-4 p-4 text-left transition-colors ${selected === index ? "bg-[var(--surface-hover)]" : "hover:bg-[var(--surface-hover)]"}`}><div className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-sm bg-[var(--surface-hover)] text-[var(--text-muted)]">{item.object_type === "task" ? <CheckSquare size={14} /> : <Database size={14} />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{name}</p>{keyField && <p className="mt-0.5 truncate font-mono text-xs text-[var(--text-muted)]">{keyField}</p>}{item.reason && <p className="mt-1 flex items-start gap-1 text-[11.5px] leading-snug" style={{ color: "var(--section-accent)" }}><Sparkles size={11} className="mt-0.5 shrink-0" />{item.reason}</p>}</div><span className="shrink-0 text-[10px] font-medium uppercase tracking-wide text-[var(--text-faint)]">{item.object_type.replaceAll("_", " ")}</span></button>; })}</div> : <div className="space-y-7">{Object.entries(grouped).map(([type, items]) => <section key={type}><h2 className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[var(--text-faint)]">{type === "contact" || type === "person" ? <UserRound size={13} /> : type === "company" ? <Building2 size={13} /> : type === "task" ? <CheckSquare size={13} /> : <Database size={13} />}{type.replaceAll("_", " ")} <span>({items.length})</span></h2><div className="minimal-sheet divide-y divide-[var(--border-soft)] overflow-hidden">{items.map((item) => { const index = flat.indexOf(item); const name = String(item.data.name ?? item.data.title ?? item.data.subject ?? "Untitled"); const keyField = String(item.data.email ?? item.data.domain ?? item.data.company ?? item.data.value ?? ""); return <button key={item.id} onClick={() => open(item)} className={`flex w-full items-center gap-4 p-4 text-left transition-colors ${selected === index ? "bg-[var(--surface-hover)]" : "hover:bg-[var(--surface-hover)]"}`}><div className="grid h-9 w-9 shrink-0 place-items-center rounded-sm bg-[var(--surface-hover)] text-[var(--text-muted)]">{item.object_type === "task" ? <CheckSquare size={14} /> : <Database size={14} />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium"><Highlight text={name} query={query} /></p><p className="mt-1 truncate font-mono text-xs text-[var(--text-muted)]"><Highlight text={keyField} query={query} /></p></div><span className="shrink-0 font-mono text-xs text-[var(--text-faint)]">{item.last_activity_at || item.updated_at ? new Date(item.last_activity_at || item.updated_at!).toLocaleDateString() : ""}</span></button>; })}</div></section>)}</div>}</div>
  </div>;
}
