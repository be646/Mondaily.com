                ...
                <div className="absolute right-0 top-8 z-30 flex gap-1.5 rounded-sm border border-[var(--border-soft)] bg-[var(--surface-card)] p-2">
                  {COLOR_KEYS.map(c => (
                    <button
                      ...
                      className={`h-5 w-5 rounded-full border-2 transition-transform hover:scale-110 ${colorKey === c ? "border-[var(--border-soft)]" : "border-transparent"} ${NOTE_COLORS[c]?.dot ?? "bg-stone-600"}`}
                    />
                  ))}
                </div>
              )}
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-x-auto rounded-sm border border-[var(--border-soft)] bg-[var(--surface-card)]">
        {BOARD_COLUMNS.map((col, laneIdx) => {
          ...
          return (
            <div key={col.key} className={laneIdx < BOARD_COLUMNS.length - 1 ? "border-b border-[var(--border-soft)]" : ""}>
              <div className="relative" style={{ width: totalWidth, height: LANE_H }}>
                ...
                {laneNotes.map(note => (
                  <div key={`dot-${note.id}`} className="absolute z-[5] h-2 w-2 -translate-x-1/2 rounded-full bg-stone-600 ring-1 ring-[var(--surface-card)]"
