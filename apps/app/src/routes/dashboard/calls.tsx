    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-1 overflow-x-auto rounded-lg border border-[var(--border-soft)] p-1">
          {(["all", "mine", "week", "month"] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`shrink-0 rounded-md px-3 py-1.5 text-sm ${filter === item ? "bg-[var(--surface-hover)] text-[var(--text-primary)]" : "text-stone-500 hover:text-stone-300"}`}>{{ all: "All", mine: "Mine", week: "This week", month: "This month" }[item]}</button>)}
        </div>
        <label className="relative block sm:w-64"><Search className="absolute left-3 top-2.5 text-stone-600" size={14} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search contact or company" className="h-9 w-full rounded-md border border-[var(--border-soft)] bg-transparent pl-9 pr-3 text-sm outline-none focus:border-[var(--border-soft)]" /></label>
      </div>
        ...
        <div className="divide-y divide-white/10 overflow-hidden rounded-lg border border-[var(--border-soft)]">
          {calls.map((call) => <Link key={call.id} to={`/calls/${call.id}`} className="flex flex-col gap-3 p-4 hover:bg-[var(--surface-hover)] sm:flex-row sm:items-center">
            <div className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <div className="min-w-0"><div className="flex items-center gap-2"><p className="truncate text-sm font-medium">{call.contact_name}</p><span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${call.direction === "inbound" ? "bg-blue-500/10 text-blue-400" : "bg-stone-500/10 text-stone-400"}`}>{call.direction === "inbound" ? "Inbound" : "Outbound"}</span></div><p className="mt-0.5 truncate text-xs text-stone-500">{call.company_name || "No linked company"}</p><p className="mt-2 truncate text-sm text-stone-400">{call.ai_summary || "Mondaily is preparing the call summary."}</p></div>
            </div>
