    ...
    <div className="space-y-5">
          ...
          {integrations.map(item => (
            <article key={item.id} className="flex flex-col bg-[#0d0f13] p-4">
              <div className="mb-3 flex items-start gap-3">
                <div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-white/[.05] text-sm font-bold text-slate-300">
                  {item.icon}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-slate-200">{item.name}</p>
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${item.connected ? "bg-emerald-500/10 text-emerald-400" : "bg-white/[.04] text-slate-600"}`}>
              ...
              <button
                ...
                className={`mt-auto self-start text-xs transition-colors ${item.connected ? "text-red-400 hover:text-red-300" : "text-emerald-400 hover:text-emerald-300"}`}
              >
      ...
      <section className="settings-section">
        <div className="settings-section-header">
          ...
          <button onClick={() => { setKeyOpen(true); setCreatedKey(""); }}
            className="flex items-center gap-1.5 rounded-lg border border-white/[.08] px-3 py-1.5 text-xs text-slate-300 hover:bg-white/[.04] hover:text-white transition-colors">
              ...
              <tbody>
                {data.api_keys.map(key => (
                  <tr key={key.id} className="border-b border-white/[.04] last:border-0 hover:bg-white/[.015] transition-colors">
                    ...
                    <td className="px-4 py-3"><code className="text-xs text-slate-600">{key.prefix}••••••••</code></td>
                    <td className="px-4 py-3 text-slate-600">{key.created_at ? new Date(key.created_at).toLocaleDateString() : "Recently"}</td>
                    <td className="px-4 py-3 text-slate-600">{key.last_used_at ? new Date(key.last_used_at).toLocaleDateString() : "Never"}</td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => revokeKey.mutate(key.id)} className="text-xs text-red-400 hover:text-red-300 transition-colors">Revoke</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {data.webhooks.map(hook => (
              <div key={hook.id} className="flex items-start justify-between gap-4 py-4">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-slate-600">{hook.events.slice(0, 3).join(", ")}{hook.events.length > 3 ? ` +${hook.events.length - 3}` : ""}</p>
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  ...
                  <button onClick={() => deleteWebhook.mutate(hook.id)} className="text-red-400 hover:text-red-300 transition-colors"><Trash2 size={13} /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="settings-section">
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm text-slate-500">Connect Mondaily to Claude, ChatGPT, and other AI tools via the Model Context Protocol.</p>
          <div>
            <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Server URL</span>
            <div className="flex items-center gap-2">
              <code className="min-w-0 flex-1 truncate rounded-lg border border-white/[.07] bg-white/[.02] px-3 py-2.5 text-xs text-slate-400">wss://mcp.mondaily.com/workspace</code>
          ...
          {data.mcp_token ? (
            <>
              <div>
                <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Token</span>
                <div className="flex items-center gap-2">
                  <code className="min-w-0 flex-1 truncate rounded-lg border border-white/[.07] bg-white/[.02] px-3 py-2.5 text-xs text-slate-400">{data.mcp_token}</code>
                  <CopyButton value={data.mcp_token} />
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => generateMcp.mutate()}
                  className="flex items-center gap-2 rounded-lg border border-white/[.08] px-3 py-2 text-xs text-slate-400 hover:bg-white/[.04] hover:text-white transition-colors">
                  <RotateCw size={12} /> Rotate token
                </button>
                <button onClick={() => toggleIntegration.mutate({ id: "mcp", connected: false })}
                  className="rounded-lg border border-red-500/20 px-3 py-2 text-xs text-red-400 hover:bg-red-500/[.08] transition-colors">
                  Revoke
                </button>
              </div>
            </>
        ...
        <ModalShell title="Create API key" close={() => setKeyOpen(false)}>
          {createdKey ? (
            <div className="space-y-4">
              ...
              <div>
                <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Your new key</span>
                <div className="flex items-start gap-2">
                  <code className="min-w-0 flex-1 break-all rounded-lg border border-white/[.07] bg-white/[.02] p-3 text-xs text-slate-300">{createdKey}</code>
                  <CopyButton value={createdKey} />
                </div>
              </div>
              <button onClick={() => setKeyOpen(false)}
                className="w-full rounded-xl bg-white/[.06] py-2.5 text-sm font-medium text-slate-300 hover:bg-white/[.09] transition-colors">
                Done
              </button>
            </div>
          ) : (
            <form onSubmit={e => { e.preventDefault(); if (keyName.trim()) createKey.mutate(); }} className="space-y-4">
              <label className="block">
                <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Key name</span>
              ...
              <button type="submit" disabled={!keyName.trim() || createKey.isPending}
                className="flex w-full items-center justify-center gap-2 rounded-xl border-x border-t border-red-500/40 border-b-[3px] border-b-red-700 bg-red-500 py-2.5 text-sm font-semibold text-white hover:bg-red-400 disabled:opacity-40 transition-all">
      ...
      {webhookOpen && (
        <ModalShell title="Add webhook" close={() => setWebhookOpen(false)}>
          <form onSubmit={e => { e.preventDefault(); if (webhook.url.startsWith("https://")) createWebhook.mutate(); }} className="space-y-4">
            <label className="block">
              <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Endpoint URL</span>
            ...
            <div>
              <span className="mb-2 block text-xs font-medium uppercase tracking-wide text-slate-500">Events to send</span>
            ...
            <div>
              <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Signing secret</span>
              <div className="flex items-center gap-2">
                <code className="min-w-0 flex-1 truncate rounded-lg border border-white/[.07] bg-white/[.02] px-3 py-2.5 text-xs text-slate-400">{webhook.secret}</code>
