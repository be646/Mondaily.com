  ...
  return (
    <div>
      ...
      <section className="premium-panel p-5">
        ...
        {calendars.length ? <div className="mb-5 space-y-2">{calendars.map((calendar) => <label key={calendar.id} className="flex items-center justify-between rounded-md bg-[var(--surface-hover)] p-3"><div><p className="text-sm">{calendar.name}</p><p className="text-xs text-stone-500">{calendar.provider}</p></div><input type="checkbox" checked={calendar.enabled} onChange={(event) => setData({ ...data, calendars: calendars.map((item) => item.id === calendar.id ? { ...item, enabled: event.target.checked } : item) })} /></label>)}</div> : <p className="mb-5 text-sm text-stone-500">Connect an email account to discover calendars.</p>}
