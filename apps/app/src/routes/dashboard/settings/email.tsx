  ...
  return (
    <div>
      ...
      <section className="premium-panel mb-5 p-5">
        <div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-medium"><Mail size={15} /> Connected email accounts</h2><div className="flex gap-2">{data.providers.filter((provider) => !provider.connected).map((provider) => <button key={provider.id} onClick={() => connect(provider.id)} className="flex items-center gap-1 rounded-md border border-white/[.06] px-2 py-1.5 text-xs"><Plus size={11} /> {provider.name}</button>)}</div></div>
        {connected.length ? <div className="space-y-3">{connected.map((provider) => <article key={provider.id} className="rounded-md border border-white/[.06] p-4">
          <div className="flex items-start gap-3"><div className="grid h-9 w-9 place-items-center rounded-md bg-indigo-500/10 text-indigo-300"><Mail size={16} /></div><div className="min-w-0 flex-1"><p className="text-sm font-medium">{provider.email ?? `${provider.name} account`}</p><p className="mt-0.5 text-xs text-slate-500">Connected as {provider.connected_as ?? provider.name}</p></div><span className={`flex items-center gap-1 rounded-full px-2 py-1 text-xs ${provider.sync_status === "error" ? "bg-indigo-500/10 text-indigo-400" : provider.sync_status === "syncing" ? "bg-amber-500/10 text-amber-400" : "bg-emerald-500/10 text-emerald-400"}`}><CheckCircle size={11} /> {provider.sync_status === "syncing" ? "Syncing" : provider.sync_status === "error" ? "Error" : `Synced ${provider.last_sync ?? "recently"}`}</span></div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-slate-500">Sync scope<select value={provider.sync_scope} onChange={(event) => updateProvider(provider.id, { sync_scope: event.target.value as Provider["sync_scope"] })} className="mt-1 h-9 w-full rounded-md border border-neutral-200 bg-white px-3 text-sm text-neutral-900 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-50"><option value="all">All mail</option><option value="inbox">Inbox only</option><option value="starred">Starred only</option></select></label><label className="text-xs text-slate-500">Email sharing<select value={provider.sharing} onChange={(event) => updateProvider(provider.id, { sharing: event.target.value as Provider["sharing"] })} className="mt-1 h-9 w-full rounded-md border border-neutral-200 bg-white px-3 text-sm text-neutral-900 dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-50"><option value="private">Private</option><option value="subject">Subject only</option><option value="full">Full email</option></select></label></div>
          <button onClick={() => updateProvider(provider.id, { connected: false })} className="mt-4 flex items-center gap-1 text-xs text-indigo-400"><Unplug size={11} /> Disconnect</button>
        </article>)}</div> : <EmptyState icon={Mail} title="No email connected" description="Connect Gmail or Outlook to sync conversations and calendars." aiHint="Connect email to let AI build context automatically." action={<button onClick={() => connect("gmail")} className="rounded-md bg-indigo-600 px-3 py-2 text-sm text-white hover:bg-indigo-700">Connect Gmail</button>} />}
      </section>

      <section className="premium-panel mb-5 p-5">
        <h2 className="mb-4 text-sm font-medium">Email preferences</h2>
        <label className="mb-5 block text-sm">Default send from<select value={data.default_from ?? connected[0]?.id ?? ""} onChange={(event) => setData({ ...data, default_from: event.target.value })} className="mt-2 h-10 w-full rounded-md border border-neutral-200 bg-white px-3 dark:border-neutral-800 dark:bg-neutral-950"><option value="">No account selected</option>{connected.map((provider) => <option key={provider.id} value={provider.id}>{provider.email ?? provider.name}</option>)}</select></label>
      ...
      <div className="mt-5 flex justify-end"><button onClick={() => save.mutate()} className="flex items-center gap-2 rounded-md bg-indigo-600 px-4 py-2 text-sm"><Save size={14} /> Save email settings</button></div>
    </div>
  );
}
