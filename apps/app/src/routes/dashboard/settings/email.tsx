    ...
    <div>
      ...
      <section className="premium-panel mb-5 p-5">
        ...
        {connected.length ? <div className="space-y-3">{connected.map((provider) => <article key={provider.id} className="rounded-md border border-white/[.06] p-4">
          <div className="flex items-start gap-3"><div className="grid h-9 w-9 place-items-center rounded-md bg-indigo-500/10 text-indigo-300"><Mail size={16} /></div><div className="min-w-0 flex-1"><p className="text-sm font-medium">{provider.email ?? `${provider.name} account`}</p><p className="mt-0.5 text-xs text-stone-500">Connected as {provider.connected_as ?? provider.name}</p></div><span className={`flex items-center gap-1 rounded-full px-2 py-1 text-xs ${provider.sync_status === "error" ? "bg-indigo-500/10 text-indigo-400" : provider.sync_status === "syncing" ? "bg-amber-500/10 text-amber-400" : "bg-emerald-500/10 text-emerald-400"}`}><CheckCircle size={11} /> {provider.sync_status === "syncing" ? "Syncing" : provider.sync_status === "error" ? "Error" : `Synced ${provider.last_sync ?? "recently"}`}</span></div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-stone-500">Sync scope<select value={provider.sync_scope} onChange={(event) => updateProvider(provider.id, { sync_scope: event.target.value as Provider["sync_scope"] })} className="mt-1 h-9 w-full rounded-md border border-stone-200 bg-white px-3 text-sm text-stone-900 dark:border-stone-800 dark:bg-stone-950 dark:text-stone-50"><option value="all">All mail</option><option value="inbox">Inbox only</option><option value="starred">Starred only</option></select></label><label className="text-xs text-stone-500">Email sharing<select value={provider.sharing} onChange={(event) => updateProvider(provider.id, { sharing: event.target.value as Provider["sharing"] })} className="mt-1 h-9 w-full rounded-md border border-stone-200 bg-white px-3 text-sm text-stone-900 dark:border-stone-800 dark:bg-stone-950 dark:text-stone-50"><option value="private">Private</option><option value="subject">Subject only</option><option value="full">Full email</option></select></label></div>
      ...
      <section className="premium-panel p-5">
        ...
        {calendars.length ? <div className="mb-5 space-y-2">{calendars.map((calendar) => <label key={calendar.id} className="flex items-center justify-between rounded-md bg-white/[.03] p-3"><div><p className="text-sm">{calendar.name}</p><p className="text-xs text-stone-500">{calendar.provider}</p></div><input type="checkbox" checked={calendar.enabled} onChange={(event) => setData({ ...data, calendars: calendars.map((item) => item.id === calendar.id ? { ...item, enabled: event.target.checked } : item) })} /></label>)}</div> : <p className="mb-5 text-sm text-stone-500">Connect an email account to discover calendars.</p>}
