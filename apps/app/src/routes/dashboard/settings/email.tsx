    ...
    <div>
      ...
      <section className="premium-panel mb-5 p-5">
        ...
        {connected.length ? <div className="space-y-3">{connected.map((provider) => <article key={provider.id} className="rounded-md border border-white/[.06] p-4">
          <div className="flex items-start gap-3"><div className="grid h-9 w-9 place-items-center rounded-md bg-stone-500/10 text-stone-300"><Mail size={16} /></div><div className="min-w-0 flex-1"><p className="text-sm font-medium">{provider.email ?? `${provider.name} account`}</p><p className="mt-0.5 text-xs text-stone-500">Connected as {provider.connected_as ?? provider.name}</p></div><span className={`flex items-center gap-1 rounded-full px-2 py-1 text-xs ${provider.sync_status === "error" ? "bg-stone-500/10 text-stone-400" : provider.sync_status === "syncing" ? "bg-amber-500/10 text-amber-400" : "bg-emerald-500/10 text-emerald-400"}`}><CheckCircle size={11} /> {provider.sync_status === "syncing" ? "Syncing" : provider.sync_status === "error" ? "Error" : `Synced ${provider.last_sync ?? "recently"}`}</span></div>
          ...
          <button onClick={() => updateProvider(provider.id, { connected: false })} className="mt-4 flex items-center gap-1 text-xs text-stone-400"><Unplug size={11} /> Disconnect</button>
        </article>)}</div> : <EmptyState icon={Mail} title="No email connected" description="Connect Gmail or Outlook to sync conversations and calendars." aiHint="Connect email to let AI build context automatically." action={<button onClick={() => connect("gmail")} className="rounded-md bg-stone-600 px-3 py-2 text-sm text-white hover:bg-stone-700">Connect Gmail</button>} />}
      </section>
