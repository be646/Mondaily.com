    ...
    <div>
      ...
      <section className="mb-5 rounded-xl border border-white/[.07] p-5">
        ...
        {connected.length ? <div className="space-y-3">{connected.map((provider) => <article key={provider.id} className="rounded-md border border-white/[.06] p-4">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-slate-500">Sync scope<select value={provider.sync_scope} onChange={(event) => updateProvider(provider.id, { sync_scope: event.target.value as Provider["sync_scope"] })} className="mt-1 h-9 w-full rounded-md border border-white/[.06] bg-[#0d0f13] px-3 text-sm text-white"><option value="all">All mail</option><option value="inbox">Inbox only</option><option value="starred">Starred only</option></select></label><label className="text-xs text-slate-500">Email sharing<select value={provider.sharing} onChange={(event) => updateProvider(provider.id, { sharing: event.target.value as Provider["sharing"] })} className="mt-1 h-9 w-full rounded-md border border-white/[.06] bg-[#0d0f13] px-3 text-sm text-white"><option value="private">Private</option><option value="subject">Subject only</option><option value="full">Full email</option></select></label></div>
        ...
        </article>)}</div> : <EmptyState icon={Mail} title="No email connected" description="Connect Gmail or Outlook to sync conversations and calendars." action={<button onClick={() => connect("gmail")} className="rounded-md bg-indigo-600 px-3 py-2 text-sm">Connect Gmail</button>} />}
      </section>

      <section className="mb-5 rounded-xl border border-white/[.07] p-5">
        <h2 className="mb-4 text-sm font-medium">Email preferences</h2>
        <label className="mb-5 block text-sm">Default send from<select value={data.default_from ?? connected[0]?.id ?? ""} onChange={(event) => setData({ ...data, default_from: event.target.value })} className="mt-2 h-10 w-full rounded-md border border-white/[.06] bg-[#0d0f13] px-3"><option value="">No account selected</option>{connected.map((provider) => <option key={provider.id} value={provider.id}>{provider.email ?? provider.name}</option>)}</select></label>
