...
export function EmailSettings() {
  ...
  return (
    <div>
      <CommandPageHeader
        variant="bar" icon={Mail} callsign="EMAIL" title="Email & calendar" subtitle="Control inbox synchronization, sending, signatures, and meeting context." />
      ...
      <section className="settings-section mb-5 p-5">
        <div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold"><Mail size={15} /> Connected email accounts</h2><div className="flex gap-2">{data.providers.filter((provider) => !provider.connected).map((provider) => <button key={provider.id} onClick={() => connect(provider.id)} className="flex items-center gap-1 rounded-sm border border-[var(--border-soft)] px-2 py-1.5 text-xs"><Plus size={11} /> {provider.name}</button>)}</div></div>
        {connected.length ? <div className="space-y-3">{connected.map((provider) => <article key={provider.id} className="rounded-sm border border-[var(--border-soft)] p-4">
          <div className="flex items-start gap-3"><div className="grid h-9 w-9 place-items-center rounded-sm bg-[var(--surface-hover)] text-[var(--text-faint)]"><Mail size={16} /></div><div className="min-w-0 flex-1"><p className="text-sm font-medium">{provider.email ?? `${provider.name} account`}</p><p className="mt-0.5 text-xs text-[var(--text-muted)]">Connected as {provider.connected_as ?? provider.name}</p></div><span className={`flex items-center gap-1 rounded-full px-2 py-1 text-xs ${provider.sync_status === "error" ? "bg-[var(--surface-hover)] text-[var(--text-faint)]" : provider.sync_status === "syncing" ? "bg-[#c6892e]/10 text-[#c6892e]" : "bg-[#2f9e6b]/10 text-[#2f9e6b]"}`}><CheckCircle size={11} /> {provider.sync_status === "syncing" ? "Syncing" : provider.sync_status === "error" ? "Error" : `Synced ${provider.last_sync ?? "recently"}`}</span></div>
          <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-[var(--text-muted)]">Sync scope<FieldSelect value={provider.sync_scope ?? "all"} onChange={v => updateProvider(provider.id, { sync_scope: v as Provider["sync_scope"] })} ariaLabel="Sync scope" className="mt-1" options={[{ value: "all", label: "All mail" }, { value: "inbox", label: "Inbox only" }, { value: "starred", label: "Starred only" }]} /></label><label className="text-xs text-[var(--text-muted)]">Email sharing<FieldSelect value={provider.sharing ?? "private"} onChange={v => updateProvider(provider.id, { sharing: v as Provider["sharing"] })} ariaLabel="Email sharing" className="mt-1" options={[{ value: "private", label: "Private" }, { value: "subject", label: "Subject only" }, { value: "full", label: "Full email" }]} /></label></div>
        ...
        </article>)}</div> : <EmptyState icon={Mail} title="No email connected" description="Connect Gmail or Outlook to sync conversations and calendars." aiHint="Connect email to let AI build context automatically." action={<button onClick={() => connect("gmail")} className="rounded-sm bg-[var(--section-accent-soft)] px-3 py-2 text-sm text-[var(--text-primary)] hover:bg-[color-mix(in_srgb,var(--section-accent)_22%,transparent)]">Connect Gmail</button>} />}
      </section>

      <SovereignInbox />

      <section className="settings-section mb-5 p-5">
        <h2 className="mb-4 text-sm font-semibold">Email preferences</h2>
        <label className="mb-5 block text-sm">Default send from<FieldSelect value={data.default_from ?? connected[0]?.id ?? ""} onChange={v => setData({ ...data, default_from: v })} ariaLabel="Default send from" className="mt-2" options={[{ value: "", label: "No account selected" }, ...connected.map((provider) => ({ value: provider.id, label: provider.email ?? provider.name }))]} /></label>
        {connected.length ? <div className="mb-5"><div className="mb-2 flex gap-1">{connected.map((provider) => <button key={provider.id} onClick={() => setSignatureAccount(provider.id)} className={`rounded-sm px-3 py-1.5 text-xs ${signatureAccount === provider.id ? "bg-[var(--surface-hover)]" : "text-[var(--text-muted)]"}`}>{provider.email ?? provider.name}</button>)}</div><SignatureEditor key={signatureAccount} content={connected.find((provider) => provider.id === signatureAccount)?.signature ?? ""} onChange={(signature) => updateProvider(signatureAccount, { signature })} /></div> : null}
      ...
      <section className="settings-section p-5">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold"><Calendar size={15} /> Calendar sync</h2>
...
function SovereignInbox() {
  ...
  return (
    <section className="settings-section mb-5 p-5">
      <h2 className="mb-1 flex items-center gap-2 text-sm font-semibold"><Mail size={15} /> Sovereign inbox</h2>
      ...
      <div className="flex items-center gap-2">
        <code className="flex-1 truncate rounded-sm border border-[var(--border-soft)] bg-[var(--surface-card)] px-3 py-2 text-[13px] text-[var(--text-primary)]">{address}</code>
        <button onClick={() => { void navigator.clipboard?.writeText(address); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
          className="rounded-sm border border-[var(--border-soft)] px-3 py-2 text-xs text-[var(--text-secondary)]">{copied ? "Copied" : "Copy"}</button>
      </div>
    </section>
  );
}
