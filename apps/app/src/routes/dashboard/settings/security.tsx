  ...
  return (
    <div>
      ...
      <section className="mb-5 rounded-lg border border-white/10 p-5">
        ...
        <label className="block text-sm">Provider<select value={data.saml_provider ?? "okta"} onChange={(event) => setData({ ...data, saml_provider: event.target.value })} className="mt-2 h-10 w-full rounded-md border border-white/10 bg-[#111419] px-3"><option value="okta">Okta</option><option value="azure">Azure AD</option><option value="google">Google Workspace</option><option value="other">Other SAML 2.0</option></select></label>
        ...
        <label className="mt-4 block text-sm">IdP metadata URL or XML<textarea value={data.saml_metadata ?? ""} onChange={(event) => setData({ ...data, saml_metadata: event.target.value })} placeholder="https://idp.example.com/metadata or paste XML" className="mt-2 min-h-24 w-full rounded-md border border-white/10 bg-transparent p-3 text-xs" /></label>
        <div className="mt-4 flex flex-wrap items-center gap-3"><button onClick={() => save({ saml_enabled: true, saml_provider: data.saml_provider, saml_metadata: data.saml_metadata })} className="rounded-md bg-red-600 px-3 py-2 text-sm">{data.saml_enabled ? "Update SSO" : "Configure SSO"}</button><button className="rounded-md border border-white/10 px-3 py-2 text-sm">Test SSO</button><label className="ml-auto flex items-center gap-2 text-sm"><input type="checkbox" checked={data.enforce_sso ?? false} onChange={(event) => save({ enforce_sso: event.target.checked })} /> Enforce SSO</label></div>
      </section>
      ...
      <section className="mb-5 rounded-lg border border-white/10 p-5">
        ...
        <form onSubmit={(event) => { event.preventDefault(); addDomain(); }} className="flex gap-2"><input value={domain} onChange={(event) => setDomain(event.target.value)} placeholder="competitor.com" className="h-9 flex-1 rounded-md border border-white/10 bg-transparent px-3 text-sm" /><button className="flex items-center gap-1 rounded-md border border-white/10 px-3 text-sm"><Plus size={13} /> Add domain</button></form>
        <div className="mt-3 flex gap-2 text-xs text-slate-500">Presets: {["gmail.com", "hotmail.com"].map((preset) => <button key={preset} onClick={() => { setDomain(preset); }} className="rounded border border-white/10 px-2 py-1">{preset}</button>)}</div>
      </section>
...
function CopyField({ label, value, copied, copy }: { label: string; value: string; copied: boolean; copy: () => void }) {
  return <label className="text-xs text-slate-500">{label}<div className="mt-1 flex h-10 items-center rounded-md border border-white/10 px-3"><code className="min-w-0 flex-1 truncate text-xs text-white">{value}</code><button onClick={copy} className="text-slate-500" aria-label={`Copy ${label}`}>{copied ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />}</button></div></label>;
}
