...
function CopyField({ label, value, copied, onCopy }: { label: string; value: string; copied: boolean; onCopy: () => void }) {
  return (
    <label className="block">
      ...
      <div className="flex h-10 items-center rounded-lg border border-white/[.08] bg-white/[.02]">
        <code className="min-w-0 flex-1 truncate px-3 text-xs text-slate-400">{value}</code>
        <button onClick={onCopy} className="px-3 text-slate-500 hover:text-white transition-colors">
    ...
    <div className="space-y-5">
      ...
      <section className="settings-section">
        <div className="settings-section-header">
          ...
          <span className={`rounded-full px-2.5 py-1 text-[10px] font-medium ${data.saml_enabled ? "bg-emerald-500/10 text-emerald-400" : "bg-white/[.04] text-slate-600"}`}>
            {data.saml_enabled ? "Configured" : "Not configured"}
          </span>
        </div>
        <div className="space-y-4 p-5">
          <p className="text-sm text-slate-500">Configure your identity provider to enable SSO login for all workspace members.</p>
          <label className="block">
            <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">Identity provider</span>
          ...
          <label className="block">
            <span className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-slate-500">IdP metadata URL or XML</span>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button className="rounded-lg border border-white/[.08] px-4 py-2 text-sm text-slate-400 hover:bg-white/[.04] hover:text-white transition-colors">
              Test SSO
            </button>
        ...
        {data.sessions.length ? (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[650px] text-left text-sm">
              ...
              <tbody>
                {data.sessions.map(s => (
                  <tr key={s.id} className={`border-b border-white/[.04] last:border-0 ${s.current ? "bg-emerald-500/[.02]" : ""}`}>
                    ...
                    <td className="px-4 py-3.5 text-slate-600">{s.browser ?? "Browser"}</td>
                    <td className="px-4 py-3.5 text-slate-600">{s.location}</td>
                    <td className="px-4 py-3.5 text-slate-600">{s.ip ?? "Hidden"}</td>
                    <td className="px-4 py-3.5 text-slate-600">{s.last_active ?? "Now"}</td>
      ...
      <section className="settings-section">
        <div className="settings-section-header">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-white"><LockKeyhole size={14} /> Protected recipients</h2>
        </div>
        <div className="p-5 space-y-4">
          <p className="text-sm text-slate-500">Prevent AI sequences from emailing specific domains. Useful for blocking competitors, personal addresses, or internal teams.</p>
          {data.protected_recipients.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {data.protected_recipients.map(r => (
                <span key={r} className="flex items-center gap-1.5 rounded-full bg-white/[.05] px-3 py-1.5 text-xs text-slate-400">
                  {r}
                  <button onClick={() => save({ protected_recipients: data.protected_recipients.filter(x => x !== r) })}
                    className="text-slate-600 hover:text-indigo-400 transition-colors"><X size={10} /></button>
                </span>
              ))}
            </div>
          )}
          <form onSubmit={e => { e.preventDefault(); addDomain(); }} className="flex gap-2">
            ...
            <button type="submit"
              className="flex items-center gap-1.5 rounded-lg border border-white/[.09] px-3 py-2 text-sm text-slate-300 hover:bg-white/[.04] hover:text-white transition-colors">
              <Plus size={13} /> Add
            </button>
          </form>
          <div className="flex items-center gap-2 text-xs text-slate-600">
            ...
            {["gmail.com", "hotmail.com", "yahoo.com"].map(p => (
              <button key={p} onClick={() => setDomain(p)}
                className="rounded-md border border-white/[.07] px-2 py-1 hover:text-slate-300 transition-colors">{p}</button>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="settings-section">
        <div className="settings-section-header">
          <h2 className="text-sm font-semibold text-white">Data & export controls</h2>
        </div>
        <div className="px-5 py-3">
          <div className="grid grid-cols-[1fr_96px_96px] items-center">
            <div className="py-2.5 text-[10px] font-semibold uppercase tracking-widest text-slate-700">Role</div>
            <div className="py-2.5 text-center text-[10px] font-semibold uppercase tracking-widest text-slate-700">CSV export</div>
            <div className="py-2.5 text-center text-[10px] font-semibold uppercase tracking-widest text-slate-700">API access</div>
            {Object.entries(data.access_controls ?? defaultControls).map(([role, controls]) => (
              <div key={role} className="contents">
                <div className="border-t border-white/[.04] py-3.5 text-sm capitalize text-slate-300">{role}</div>
                <div className="border-t border-white/[.04] py-3.5 flex justify-center">
                  <AccessToggle
            ...
            <table className="w-full min-w-[600px] text-left text-sm">
              ...
              <tbody>
                {data.audit_log?.map(entry => (
                  <tr key={entry.id} className="border-b border-white/[.04] last:border-0">
                    <td className="px-4 py-3 text-slate-200">{entry.actor}</td>
                    <td className="px-4 py-3 font-mono text-xs text-slate-400">{entry.action}</td>
                    <td className="px-4 py-3 text-slate-600">{entry.target}</td>
