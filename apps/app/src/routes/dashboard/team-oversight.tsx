...
interface ActorTally { actor_id: string; actor_type: string; actor_name: string | null; actor_avatar: string | null; count: number }
interface MembersData { members: { email?: string; role?: string }[] }

// Resolve any actor (human / agent / system) → a consistent display identity.
function actorIdentity(r: { actor_type: string; actor_id: string | null; actor_name: string | null }) {
  if (r.actor_type === "agent") { const a = agentByRaw(r.actor_id ?? ""); return { name: a.name, isAgent: true }; }
  if (r.actor_type === "system") return { name: "System", isAgent: false };
  return { name: r.actor_name || "Unknown user", isAgent: false };
}
...
function clock(iso: string) { return new Date(iso).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" }); }
...
export function TeamOversightPage() {
  ...
  // Role gate (client-side, for a clean denied state). The backend ALSO enforces
  // owner/admin on /activities/oversight — this is defense-in-depth, not the only gate.
  const { data: membersData } = useQuery<MembersData>({ queryKey: ["members"], queryFn: () => apiClient.get<MembersData>("/settings/members") });
  const myEmail = user?.primaryEmailAddress?.emailAddress?.toLowerCase();
  const myRole = membersData?.members.find(m => m.email?.toLowerCase() === myEmail)?.role ?? "member";
  const isAdmin = myRole === "owner" || myRole === "admin";

  const { data, isLoading, isError } = useQuery<{ activity: OversightRow[]; actors: ActorTally[] }>({
  ...
  // Access-denied — non-managers (and a defensive catch if the server 403s).
  if ((membersData && !isAdmin) || isError) {
    return (
  ...
  const rows = (data?.activity ?? []).filter(r => !actorFilter || String(r.actor_id) === actorFilter);
      ...
      {actors.length > 0 && (
        <div className="mb-5 flex flex-wrap gap-2">
          {actors.map(a => {
            ...
            return (
              <button key={a.actor_id || a.actor_type} onClick={() => setActorFilter(on ? null : a.actor_id)}
                ...
                <Avatar avatar={a.actor_avatar} isAgent={id.isAgent} name={id.name} actorId={a.actor_id} />
                <span style={{ color: "var(--text-secondary)" }}>{id.name}</span>
                <span className="tabular-nums" style={{ color: "var(--text-faint)" }}>{a.count}</span>
              </button>
            );
          })}
        </div>
            ...
            <div key={g.day}>
              ...
              <div className="surface-card overflow-hidden rounded-2xl">
                {g.items.map((r, i) => {
                  ...
                  return (
                    <div key={r.id} className="flex items-start gap-3 px-4 py-3" style={i > 0 ? { borderTop: "1px solid var(--border-soft)" } : undefined}>
                      <Avatar avatar={r.actor_avatar} isAgent={id.isAgent} name={id.name} actorId={r.actor_id} />
