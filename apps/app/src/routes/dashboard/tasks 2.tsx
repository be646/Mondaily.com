...
const BTN_PRIMARY = "flex-1 h-10 rounded-xl bg-red-600 text-sm font-medium text-white disabled:opacity-50 hover:bg-red-500 transition-colors";
...
function DraggableCard({ task, onDetail, onEdit, onDelete, onToggle, currentUserId, getMemberName }: {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}
      className={`rounded-xl border p-3 transition-all ${isDragging ? "shadow-2xl opacity-80 border-red-500/40 bg-[#1a1d24]" : "border-white/[.07] bg-white/[.02] hover:border-white/[.12]"}`}>
      ...
      <div className="relative" style={{ zIndex: 1 }}>
        <div className="flex items-start gap-2 mb-2">
          <button onPointerDown={e => e.stopPropagation()} onClick={() => onToggle(task)}
            className={`mt-0.5 h-4 w-4 shrink-0 rounded border flex items-center justify-center transition-colors ${task.completed ? "border-emerald-500 bg-emerald-500" : isOverdue ? "border-red-400/60 hover:border-red-400" : "border-white/25 hover:border-white/50"}`}>
          ...
          <div className="flex gap-0.5 shrink-0">
            ...
            {(task.assignee_id === currentUserId || !task.assignee_id) && (
              <button onPointerDown={e => e.stopPropagation()} onClick={() => onDelete(task.id)}
                className="rounded-md p-1 text-slate-600 hover:text-red-400 hover:bg-red-400/10 transition-colors"><Trash2 size={10}/></button>
            )}
          </div>
        </div>
        ...
        {(assigneeName || task.due_date) && (
          <div className="mt-2 flex items-center gap-2 text-[10px] text-slate-600">
            ...
            {task.due_date && <span className={`flex items-center gap-0.5 ${isOverdue ? "text-red-400" : ""}`}><Clock size={9}/>{fmtDate(task.due_date)}</span>}
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col min-w-[240px] w-[240px] shrink-0">
      <div className="flex items-center gap-2 mb-3 px-1">
        ...
        <span className="ml-auto rounded-full bg-white/[.06] px-2 py-px text-[10px] text-slate-500">{tasks.length}</span>
      </div>
      <div ref={setNodeRef}
        className={`flex-1 min-h-[120px] rounded-xl border-2 border-dashed p-2 space-y-2 transition-colors ${isOver ? "border-red-500/30 bg-red-500/[.03]" : "border-white/[.04] bg-white/[.01]"}`}>
...
function AISuggestModal({ onClose, members, currentUserId }: { onClose: () => void; members: Member[]; currentUserId: string }) {
  ...
  const PCOL: Record<string, string> = { low: "text-slate-400", medium: "text-blue-400", high: "text-orange-400", urgent: "text-red-400" };
...
export function TasksPage() {
  ...
  const BOARD_COLS: { key: string; label: string; dotColor: string }[] = [
    { key: "todo",        label: "To Do",       dotColor: "bg-slate-500"   },
      ...
      <div className="mb-5 flex items-center gap-1.5 flex-wrap">
        ...
        <div className="flex gap-0.5 rounded-xl border border-white/[.07] bg-white/[.02] p-0.5">
          {([
            ...
            <button key={f.key} onClick={() => setFilter(f.key)}
              ...
              {f.label}
              {f.badge && filter !== f.key && <span className="rounded-full bg-red-500/20 px-1 py-px text-[10px] text-red-400">{f.badge}</span>}
            </button>
          ))}
        </div>
        ...
        <div className="relative">
          ...
          <button onClick={() => { setLabelOpen(o => !o); setPriorityOpen(false); setSortOpen(false); }}
            className={`flex items-center gap-1 rounded-xl border px-2.5 py-1 text-xs transition-colors ${labelFilter ? "border-red-500/30 bg-red-500/[.06] text-red-400" : "border-white/[.07] text-slate-500 hover:text-slate-300 hover:border-white/[.12]"}`}>
        ...
        <div className="relative">
          ...
          <button onClick={() => { setPriorityOpen(o => !o); setLabelOpen(false); setSortOpen(false); }}
            className={`flex items-center gap-1 rounded-xl border px-2.5 py-1 text-xs transition-colors ${priorityFilter ? "border-red-500/30 bg-red-500/[.06] text-red-400" : "border-white/[.07] text-slate-500 hover:text-slate-300 hover:border-white/[.12]"}`}>
        ...
        <div className="relative">
          ...
          <button onClick={() => { setSortOpen(o => !o); setLabelOpen(false); setPriorityOpen(false); }}
            className={`flex items-center gap-1 rounded-xl border px-2.5 py-1 text-xs transition-colors ${(sortBy !== "created_at" || sortDir !== "desc") ? "border-red-500/30 bg-red-500/[.06] text-red-400" : "border-white/[.07] text-slate-500 hover:text-slate-300 hover:border-white/[.12]"}`}>
            <ArrowUpDown size={11}/>
            ...
            <div className="dropdown-panel left-0 w-48 z-50">
              {[
                ...
                <button key={opt.value} onClick={() => { setSortBy(opt.value); setSortOpen(false); }}
                  ...
                  {opt.icon}{opt.label}{sortBy === opt.value && <Check size={12} className="ml-auto text-red-400"/>}
                </button>
              ))}
              <div className="mx-2 my-1 border-t border-white/[.07]"/>
              <div className="flex gap-1 px-1 pb-1">
                <button onClick={() => setSortDir("desc")} className={`flex flex-1 items-center justify-center gap-1 rounded-lg py-1.5 text-xs transition-colors ${sortDir === "desc" ? "bg-white/[.06] text-white" : "text-slate-500 hover:bg-white/[.04] hover:text-slate-300"}`}><ArrowDown size={11}/> Newest</button>
                <button onClick={() => setSortDir("asc")}  className={`flex flex-1 items-center justify-center gap-1 rounded-lg py-1.5 text-xs transition-colors ${sortDir === "asc"  ? "bg-white/[.06] text-white" : "text-slate-500 hover:bg-white/[.04] hover:text-slate-300"}`}><ArrowUp size={11}/> Oldest</button>
              </div>
            </div>
          )}
        </div>
        ...
        query.isLoading ? <PageSkeleton/> : query.isError ? <ErrorState error={query.error as Error} onRetry={() => query.refetch()}/> : tasks.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {tasks.map(task => {
              ...
              return (
                <div key={task.id} className={`rounded-2xl border transition-colors ${isOverdue ? "border-red-500/20 bg-red-500/[.03]" : "border-white/[.07] bg-white/[.02] hover:border-white/[.11]"}`}>
                  <div className="flex items-center gap-3 px-4 py-3">
                    {/* Checkbox */}
                    <button onClick={() => toggle.mutate(task)}
                      className={`h-5 w-5 shrink-0 rounded border flex items-center justify-center transition-colors ${task.completed ? "border-emerald-500 bg-emerald-500" : isOverdue ? "border-red-400/60 hover:border-red-400" : "border-white/25 hover:border-white/50"}`}>
                    ...
                    <div className="flex items-center gap-0.5 shrink-0">
                      ...
                      {(task.assignee_id === currentUserId || !task.assignee_id) && (
                        <button onClick={() => setConfirmDeleteId(task.id)} title="Delete"
                          className="rounded-lg p-1.5 text-slate-600 hover:text-red-400 hover:bg-red-400/10 transition-colors"><Trash2 size={12}/></button>
                      )}
                    </div>
                  </div>
                  ...
                  {expanded && (
                    <div className="border-t border-white/[.06] px-4 py-4 space-y-3">
                      <div className="flex flex-wrap gap-4 text-xs">
                        <div><p className="text-slate-600 mb-0.5">Created</p><p className="text-slate-300">{task.created_at ? fmtDateTime(task.created_at) : "—"}</p></div>
                        <div><p className="text-slate-600 mb-0.5">Due</p><p className={task.due_date ? (isOverdue ? "text-red-400" : "text-slate-300") : "text-slate-600"}>{task.due_date ? fmtDateTime(task.due_date) : "No due date"}</p></div>
                        <div><p className="text-slate-600 mb-0.5">Assignee</p><p className="text-slate-300">{assigneeName || "Unassigned"}</p></div>
                        {task.record_name && <div><p className="text-slate-600 mb-0.5">Record</p><p className="text-slate-300">{task.record_name}</p></div>}
                      </div>
                      ...
                      {task.notes && <p className="text-sm text-slate-400 leading-relaxed whitespace-pre-wrap">{task.notes}</p>}
                    </div>
                  )}
                </div>
              );
                    ...
                    return (
                      <tr key={task.id} className="group hover:bg-white/[.015] transition-colors">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-xs tabular-nums">
                          {task.due_date ? <span className={isOverdue ? "text-red-400" : "text-slate-400"}>{fmtDate(task.due_date)}</span> : <span className="text-slate-700">—</span>}
                        </td>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {(task.assignee_id === currentUserId || !task.assignee_id) && (
                              <button onClick={() => setConfirmDeleteId(task.id)} className="rounded-md p-1 text-slate-600 hover:text-red-400 hover:bg-red-400/10 transition-colors"><Trash2 size={12}/></button>
                            )}
                          </div>
                        </td>
                      </tr>
            ...
            <div className="space-y-1 opacity-50">
              {doneTasks.map(task => (
                <div key={task.id} className="rounded-xl border border-white/[.04] p-3 flex items-center gap-3">
                  ...
                  <button onClick={() => setDetailTask(task)} className="flex-1 text-sm text-slate-500 line-through text-left hover:text-slate-400 truncate transition-colors">{task.title}</button>
                  <span className="text-[10px] text-slate-700 shrink-0">{task.status === "done" ? "Done" : "Completed"}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl border border-white/[.09] bg-[#0d0f13] p-6 shadow-2xl">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-red-500/10 mb-4">
